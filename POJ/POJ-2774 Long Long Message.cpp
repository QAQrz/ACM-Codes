...
const LL inf=0x3f3f3f3f,mod=1e9+7,maxn=212345,logmaxn=22,maxc=128;
struct sa{
...
int main(){
	...
	for(int i=0;i<l+l1+2;i++){
		if(i&&((sa.sa[i-1]<l&&sa.sa[i]>l)||(sa.sa[i-1]>l&&sa.sa[i]<l)))
			ans=max(ans,sa.height[i]);
		if(i<l+l1+1&&((sa.sa[i]<l&&sa.sa[i+1]>l)||(sa.sa[i]>l&&sa.sa[i+1]<l)))
			ans=max(ans,sa.height[i+1]);
	}
	db(ans);
	return 0;
}